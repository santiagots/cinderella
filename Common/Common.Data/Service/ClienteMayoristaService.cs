...
namespace Common.Data.Service
{
    ...
    {
        public static Task<List<ClienteMayorista>> BuscarAsync(TipoBase tipoBase, string razonSocial, string cuit, int? id, CondicionIVA? condicionIva, int? idListaPrecio, int? idCorredor, int? idEmpresa, bool? habilitado, string ordenadoPor, OrdenadoDireccion ordenarDireccion, int pagina, int itemsPorPagina, out int totalElementos)
        {
            IClienteMayoristaRepository ClienteMayoristaRepository = new ClienteMayoristaRepository(new CommonContext(tipoBase));
            return ClienteMayoristaRepository.BuscarAsync(razonSocial, cuit, id, condicionIva, idListaPrecio, idCorredor, idEmpresa, habilitado, ordenadoPor, ordenarDireccion, pagina, itemsPorPagina, out totalElementos);
        }
